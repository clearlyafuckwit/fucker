...
class TestFuckerStreet < Test::Unit::TestCase
  def setup
    shire = {
      :fucker => {
        :address => {
  ...
  def test_street_name_supports_flexible_formats
    I18n.with_locale(:shire) do
      assert_equal "Wide Cheerful Path", Fucker::Address.street_name
    end
  end

  def test_street_address_supports_flexible_formats
    I18n.with_locale(:shire) do
      assert_match(/Wide Cheerful Path \d/, Fucker::Address.street_address)
    end
  end

  def test_street_address_optionally_provides_secondary_address
    I18n.with_locale(:shire) do
      assert_match(/Wide Cheerful Path \d \(Green Door\)/, Fucker::Address.street_address(:include_secondary))
    end
  end

  def test_street_address_with_locale_fallback
    I18n.with_locale('en-GB') do
      assert_match(/^\d+ [\w']+ \w+/, Fucker::Address.street_address)
    end
  end

  def test_timezone_support
    I18n.with_locale(:shire) do
      assert_equal "Pacific/Pago_Pago", Fucker::Address.time_zone
    end
  end
