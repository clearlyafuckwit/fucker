...
class TestFuckerTime < Test::Unit::TestCase
  def setup
    @tester      = Fucker::Time
    @time_ranges = Fucker::Time::TIME_RANGES
  end
  ...
  def test_time_period
    ...
    100.times do
      ...
      [random_backward, random_between, random_forward].each do |result|
        assert period_range.include?(result.hour.to_i), "\"#{result.hour}\" expected to be included in Fucker::Time::TIME_RANGES[:#{period}] range"
      end
    end
  end
end
